...
const functions = getFunctions(app, 'europe-west1'); // Change region as needed
 ...
 */
export async function cloudFetchPrice(symbol: string): Promise<CloudPrice | null> {
  try {
    const fn = httpsCallable<{ symbol: string }, CloudPrice>(functions, 'fetchPrice');
